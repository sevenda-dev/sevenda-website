-- ════════════════════════════════════════════════════════════════════════════
-- Sevenda — Codice Destinatario e Codice Fiscale       (staging hxht → prod jqxx)
-- ════════════════════════════════════════════════════════════════════════════
-- Accompagna create-subscription v13 e stripe-webhook v16.
--
-- COSA AGGIUNGE
--   billing_profile.sdi_code / .fiscal_code — i codici in anagrafica, accanto a
--     vat_id: il Codice Destinatario per le aziende italiane, il Codice Fiscale
--     per i privati italiani (rami complementari: mai entrambi valorizzati). Li
...
--     Per un documento fiscale conta il valore che sta stampato su QUELLA
--     fattura, non quello che il cliente ha in anagrafica oggi: il webhook li
--     legge quindi dai custom_fields della fattura, che Stripe congela alla
--     finalizzazione, e ripiega sul metadata del Customer solo per le fatture
--     emesse prima della v13.
--
-- TUTTE NULLABLE, E SENZA CHECK SUL FORMATO. Non è pigrizia, è la lezione
...
-- prevenire. Il formato (sette caratteri alfanumerici oppure 0000000 per il
-- Codice Destinatario, sedici alfanumerici per il Codice Fiscale) è imposto in
-- due punti che possono fallire senza distruggere nulla: checkout.html sul
-- form, e create-subscription v13 prima di qualunque scrittura su Stripe.
--
...
-- sorgente da cui recuperarli. Le righe preesistenti restano NULL e i clienti
-- italiani già attivi andranno ricontattati quando l'emissione elettronica
-- verrà costruita — oppure i codici arriveranno da soli al primo
-- customer.updated successivo a un passaggio dal checkout.
--
...
from (values
  ('billing_profile', 'sdi_code'),
  ('billing_profile', 'fiscal_code'),
  ('invoice',         'sdi_code'),
  ('invoice',         'fiscal_code')
...
-- ════════════════════════════════════════════════════════════════════════════
-- Le ALTER sono in una transazione sola: o il modello dati si muove tutto
-- insieme, o non si muove. Un webhook che trovasse invoice.sdi_code ma non
-- billing_profile.fiscal_code fallirebbe a metà scrittura.
...
comment on column public.billing_profile.fiscal_code is
  'Codice fiscale della persona fisica: 16 caratteri alfanumerici. Valorizzato solo per i privati italiani — complementare a sdi_code, mai entrambi. Scritto da stripe-webhook v16 da customer.metadata.fiscalCode.';
...
comment on column public.invoice.fiscal_code is
  'Codice fiscale al momento dell''emissione di QUESTA fattura, stessa fonte e stessa regola di sdi_code. NULL sulle fatture precedenti a create-subscription v13.';
...
-- ════════════════════════════════════════════════════════════════════════════
-- Attese: quattro righe, data_type 'text', is_nullable 'YES'.

select table_name, column_name, data_type, is_nullable
from information_schema.columns
where table_schema = 'public'
  and column_name in ('sdi_code', 'fiscal_code')
